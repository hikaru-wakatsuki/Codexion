...
- `dongle->mutex` — protects `owner_coder_id`, `cooldown_until_ms`, `local_seq`, and `wait_queue` for each dongle. Acquired in index order (first dongle index < second) to prevent deadlock.
...
When a coder checks whether it can take both dongles, it holds both `first->mutex` and `second->mutex` simultaneously before reading any state. This atomic check-and-acquire prevents two coders from both passing the `can_take` check and both claiming the same dongle:
